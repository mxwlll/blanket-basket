...
// Define packages (could be moved to a shared file if used elsewhere)
const packages = {
  'classic-picnic': {
    name: 'Classic Picnic',
    price: '$150',
...
// Define locations (could be moved to a shared file)
const locations = [
  {
    id: 'sheep-meadow',
    name: 'Sheep Meadow',
...
export default function PurchaseContent() {
  ...
  useEffect(() => {
    if (packageId && packages[packageId as keyof typeof packages]) {
      ...
      // Handle invalid or missing packageId, maybe redirect or show an error
      console.error('Invalid or missing package ID:', packageId);
      // Example: redirect to home if package is invalid after a short delay
  ...
  // Loading state handled by Suspense fallback in page.tsx
  // if (loading) { ... }
  ...
  if (!selectedPackage) {
    // This case might be hit briefly if loading finishes before package is set
    // Or if the package ID was genuinely invalid
    // The Suspense fallback in page.tsx handles the initial loading
    return null; // Or a more specific error/loading state if preferred
  }
          ...
          <div className="bg-white rounded-lg shadow-lg overflow-hidden sticky top-6">
            <div className="relative h-48">
              <Image
                ...
                fill
                style={{ objectFit: 'cover' }} // Use style prop for fill effect
              />
            </div>
            <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      name="guests"
                      min="1"
                      max="10" // Consider making this dynamic based on package
